<template>
  ...
  >
    ...
    <div class="controls">
        ...
        >
          ...
          >
            ...
            <div class="flex-column content">
              <FormKit :disabled="!loaded" type="group">
                <FormKit
                  v-model="textureInputForm"
                  :disabled="!loaded"
                  type="group"
                >
                  ...
                  <FormKit
                    ...
                    help="Textur oder Hotspot-Datei auswählen"
      ...
      <Transition
        v-if="screenSize.height > totalPlanetCount * 30 + 10 + 380 && !isMobile"
        enter-active-class="animate__animated animate__fadeInLeftBig"
        mode="in-out"
      >
        <ul v-if="showOverlays" id="planets" v-auto-animate>
          <template v-for="planet in planets" :key="planet.uuid">
            ...
            <li
              v-if="planet.enabled"
              :class="{
                active: planet.uuid === currentPlanet.uuid,
                disabled: !loaded,
                parent: planet.moons.length > 0,
              }"
              class="planet-selector"
            >
              <span @click="changePlanet(planet)">{{ planet.name }}</span>
              <span class="resolution_badge">{{ planet.resolution }}k</span>
            </li>
            <template v-for="(moon, index) in planet.moons" :key="moon.uuid">
              <!--skipcq: JS-V001-->
              <li
                v-if="moon.enabled && moon.uuid !== currentPlanet.uuid"
                :class="{
                  active: moon.uuid === currentPlanet.uuid,
                  disabled: !loaded,
                  parentActive: planet.uuid === currentPlanet.uuid,
                  last: index === planet.moons.length - 1,
                }"
                class="planet-selector moon"
              >
                <span @click="changePlanet(moon)">{{ moon.name }}</span>
                <span class="resolution_badge">{{ moon.resolution }}k</span>
              </li>
            </template>
          </template>
        </ul>
        ...
        <div
          v-if="
            !(
              hotspots.length === 0 ||
              !loaded ||
              (lastHotspot.position.x === 0 &&
                lastHotspot.position.y === 0 &&
                lastHotspot.position.z === 0)
            ) && sidePanelType === 'hotspotSettings'
          "
          class="wrapper"
        >
          <FormKit
          ...
          >
            ...
            <p v-if="windowHeight >= 913" style="max-width: 225px">
              Direkt nach dem hinzufügen (<kbd>Alt + Klick auf den Planeten</kbd
              >) hier die Einstellungen vornehmen.<br />
              Um einen Hotspot zu bearbeiten, einfach (mit gedrückter
              <kbd>Strg</kbd>-Taste) auf den Hotspot klicken.
            </p>
            <FormKit
      ...
      >
        <div
          v-if="sidePanelType === 'planetInfo' && showOverlays"
          id="planetInfo"
          class="wrapper"
        >
          <div
            class="iconHolder"
            @click="planetInfoCollapsed = !planetInfoCollapsed"
          >
            <Icon class="collapseIcon" icon="ph:caret-double-right-bold" />
          </div>
          <div class="content">
            <span
              v-if="currentPlanet.mag !== 1.00001"
              class="mag_badge"
              title="Magnitude (scheinbare Helligkeit)"
              >Mag: {{ round(currentPlanet.mag, 2) }}</span
            >
            <h2 :title="planetInfo.nameTooltip" class="name">
              {{ planetInfo.name }}
            </h2>
            <div class="description">
              ...
              >
                <template v-if="connectionAlive">
                  ...
                  <br />
                  <!-- // skipcq: JS-0693 -->
                  <p v-if="planetInfo.link">
                    Quelle:
                    <a
                      :href="planetInfo.link"
                      rel="noopener noreferrer"
                      target="_blank"
                      >{{ planetInfo.linkText }}</a
                    >
                  </p>
                </template>
                <template v-else>
            ...
            <template
              v-if="
                planetInfo.detailed !== undefined &&
                Object.keys(planetInfo.detailed).length > 0
              "
            >
              <hr />
              <div
                v-for="(value, key) in planetInfo.detailed"
                :key="key"
                class="dropdowns"
              >
                <Dropdown
                  :open-override="openPlanetInfoDropdown === key"
                  :title="key"
                  @close="openPlanetInfoDropdown = 'none'"
                  @open="openPlanetInfoDropdown = key"
                >
            ...
            <Dropdown
              :open-override="openPlanetInfoDropdown === 'appinfo'"
              italic
              title="App-Info"
              @close="openPlanetInfoDropdown = 'none'"
              @open="openPlanetInfoDropdown = 'appinfo'"
            >
              <p>
                Die App wurde von <u>Dominik Fuchs</u> entwickelt und ist Open
                Source. Der Quellcode ist auf
                <a
    ...
    <button
      ...
      :key="hotspot.uuid"
      :slot="'hotspot-' + hotspot.type + '-' + hotspot.uuid"
      :class="[hotspot.class, { action: hotspot.action }]"
  ...
  computed: {
    ...
    planetInfo() {
      if (
        ...
        this.currentPlanet.key === "empty"
      ) {
        return {
          name: "Kein Planet ausgewählt",
          description:
            "Wähle einen Planeten aus, um mehr Informationen zu erhalten.",
  ...
  methods: {
    ...
    errorHandler(error) {
      ...
      switch (error.detail.type) {
        ...
        case "networkerror":
          messageContent =
            "Das Modell konnte nicht geladen werden. Bitte überprüfe deine Internetverbindung.";
          break;
      }
      const defaultAppend =
        "Das neu laden der Seite könnte helfen. Andernfalls versuche es später noch einmal.";
      this.addMessage(
        "Fehler",
    ...
    addHotspot(event) {
      // todo bug, when adding new hotspot and panel is closed, last one will be overwritten
      const condition =
        (this.hotspots.length === 0 &&
          this.lastHotspot.position.x === 0 &&
          this.lastHotspot.position.y === 0 &&
          this.lastHotspot.position.z === 0) ||
    ...
    updateLastHotspot() {
      ...
      this.lastHotspot.class =
        this.lastHotspot.type === "location"
          ? "location level-" + this.lastHotspot.level
          : this.lastHotspot.type;
...
html[data-theme="dark"] #settingsCollapse:hover {
  filter: brightness(0.4);
}

#planetInfo.collapsed .iconHolder,
#settings.collapsed .iconHolder {
...
#settings.collapsed .content {
  opacity: 0;
}

#planetInfo.fullyCollapsed .content,
#settings.fullyCollapsed .content {
  display: none;
}
...
li.planet-selector:first-of-type.active:not(.parent),
li.planet-selector.moon.parentActive.last {
  margin-bottom: 10px;
}
...
.hotspot:not([data-visible]) {
  /*todo absprache mit Louis, wie dke verschiedenen Hotspots dargestellt werden*/
...
.hotspot.location[data-visible],
.hotspot.location[data-visible] .hotspot-annotation {
  opacity: calc(
    var(--max-hotspot-opacity) * (1 / var(--hotspot-zoom)) *
      var(--levelOpacityMultiplier)
  ) !important;
}

#planet[data-settings-active] .hotspot,
#planet[data-settings-active] .hotspot-annotation {
