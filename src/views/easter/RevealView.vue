<template>
  <div id="all" v-if="!admin">
    <main>
      <div id="showAnswer" v-if="showAnswer">
        <span id="pre">The answer is: </span><br /><span id="answer">{{
          answer
        }}</span>
      </div>
  ...
  <div id="all" v-else>
    <main>
      ...
      <select v-model="answerIn">
        <option v-for="(value, key) in possibleAnswers" :value="key">
          {{ value }}
        </option>
      </select>
      <button @click="generateToken">Generate Token</button>
      <span id="pre">The token is: </span
      ><span id="answer">{{ generatedToken }}</span>
    </main>
  </div>
</template>
  ...
  computed: {
    answer() {
      ...
      return this.possibleAnswers[
        this.token.substring(indexOfFirstDash + 3, indexOfFirstDash + 5)
      ];
    },
    tokenDate() {
      let indexofFirstDash = this.token.indexOf("-");
